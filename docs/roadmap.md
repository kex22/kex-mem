...
功能：
- longmem init（目录结构、CLAUDE.md 注入、SQLite FTS5）
- longmem log（每日日志写入 + 索引更新）
- longmem search（FTS5 BM25 搜索）
- longmem recall（查看日志 / MEMORY.md）
- longmem index（重建索引）
- longmem compact（输出旧日志供整理 + --auto 归档）
...
- `longmem compact --smart`：调用 LLM 自动提炼旧日志到 MEMORY.md
